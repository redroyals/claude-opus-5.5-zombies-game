...
const BUDGET = { weapons: [600e3, 7000, 1024], attachments: [250e3, 3000, 512], equipment: [250e3, 2500, 512], characters: [1500e3, 18000, 2048], kits: [1000e3, 6000, 1024] };
const framesF = path.join(ROOT, 'assets/weapons/frames.json');
...
async function one(a) {
  ...
  const outDir = path.join(ROOT, 'assets', a.cat === 'kits' ? `kits/${a.map}` : a.cat); fs.mkdirSync(outDir, { recursive: true });
  ...
  const bargs = ['-b', '--factory-startup', '-P', path.join(ROOT, 'tools/blender/postprocess.py'), '--', '--in', src, '--out', wGlb, '--cat', a.cat, '--size', String(a.size), '--tris', String(tris), '--frame', wFrame, '--name', a.id];
  if (a.cls) bargs.push('--cls', a.cls);
  if (BULLPUP.has(a.id)) bargs.push('--bullpup', '1');
  if (ov.grip !== undefined) bargs.push('--grip', String(ov.grip));
  if (ov.flip) bargs.push('--flip', '1');
  if (a.cat === 'kits' || a.cat === 'weapons') bargs.push('--lod1', wLod);
