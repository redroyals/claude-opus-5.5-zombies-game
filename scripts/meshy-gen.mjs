...
const FLOOR = Number(process.env.MESHY_FLOOR ?? 2960); // 10960 start - 9000 cap + 1000 headroom kept unspent
...
if (import.meta.url === `file://${process.argv[1]}`) {
  let list = ALL.filter((a) => (!args.cat || a.cat === args.cat) && (!args.ids || String(args.ids).split(',').includes(a.id)) && (!args.map || a.map === args.map));
  ...
  await Promise.all(Array.from({ length: conc }, async () => {
    while (i < list.length) {
      const a = list[i++];
      if (!state[a.id]?.task && !state[a.id]?.preview) { const bal = await balance(); if (bal - 40 * conc < FLOOR) { console.log('BUDGET STOP at', bal); i = list.length; return; } }
      await gen(a);
    }
  }));
  const b1 = await balance();
  fs.appendFileSync(path.join(ROOT, 'assets/LOG.md'), `\n- ${new Date().toISOString()} batch \`${process.argv.slice(2).join(' ')}\`: ${list.length} assets, balance ${b0} -> ${b1} (spent ${b0 - b1})\n`);
