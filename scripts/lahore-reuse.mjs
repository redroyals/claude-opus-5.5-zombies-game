...
const out = path.join(ROOT, 'public/models/lahore/reuse'); // the flag is loaded directly
const srcOut = path.join(ROOT, 'public/models/lahore/src/reuse'); // the rest are atlas sources (scripts/lahore-atlas.mjs)
fs.mkdirSync(out, { recursive: true }); fs.mkdirSync(srcOut, { recursive: true });
for (const [id, metal] of Object.entries(USE)) {
  ...
  fs.writeFileSync(path.join(id.startsWith('flag') ? out : srcOut, `${id}.glb`), buf);
