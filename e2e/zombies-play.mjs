...
// The Cache is hidden at the start and surfaces on the dock once two doors are open.
console.log('cache at start', JSON.stringify((await ds('zm')).box));
await ds('zOpen', 'lobby_hall');
await until((z) => z.box.phase !== 'hidden', 10000);
const zb = await ds('zm');
const bs = (await ds('zDef')).box.spots[zb.box.location];
console.log('cache surfaced', JSON.stringify(zb.box));
const bf = { x: bs.x + Math.sin(bs.face) * 1.3, z: bs.z + Math.cos(bs.face) * 1.3 };
const byaw = Math.atan2(-(bs.x - bf.x), -(bs.z - bf.z));
await ds('teleport', bf.x, bf.z, byaw);
await ds('look', byaw, -0.25);
await wait(300);
await press();
...
for (const d of ['dock_hall', 'dock_vault', 'hall_power']) await ds('zOpen', d);
